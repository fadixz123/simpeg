...
function DrawChart($chart){
	...
	if(!isset($chart[ 'canvas_bg' ]['height' ])){$chart[ 'canvas_bg' ]['height' ] =300;}
	if(!isset($chart[ 'canvas_bg' ]['color' ])){$chart[ 'canvas_bg' ]['color' ] ="666666";}
								
	$params=GetParams($chart);
	if(isset($chart[ 'param_file' ])){
		if(gettype($chart[ 'param_file' ])=="array" ){
			if(!isset($chart[ 'param_file' ][ 'write' ]) or $chart[ 'param_file' ][ 'write' ]){file_write($path.$chart[ 'param_file' ][ 'url' ], $params);}
			if(!isset($chart[ 'param_file' ][ 'read' ]) or $chart[ 'param_file' ][ 'read' ]){$params="param_file=".$chart[ 'param_file' ][ 'url' ]."&";}
		}else{
